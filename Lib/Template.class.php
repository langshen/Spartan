...
class Template
{
                ...
                switch (strtolower($fun)) {
                    ...
                    default: // 通用模板函数
                        if (isset($args[1])) {
                            ...
                            if (!empty($args[0])) {
                                if ($args[0]==$this->config['default_filter'] && substr($name,-1,1) != ')'){
    ...
    private function parseTemplateFile($template)
    {
        ...
        $strModule = \Spt::$arrConfig['CONTROL'];// 获取当前模块
        if(strpos($template,'@')){ //跨模块调用模版文件
            $arrTmp = explode('@',$template);
            $strModule = ucfirst($arrTmp[0]);
            unset($arrTmp[0]);
            $template = implode(DS,$arrTmp);
        }else{
            $template = $template?$template:strtolower(\Spt::$arrConfig['ACTION']);
        }
        $template = config('APP_PATH').'View'.DS.$strModule.DS.$template.'.html';
        if (is_file($template)) {
            $this->includeFile[$template] = filemtime($template);
            return $template;
        }else{
